...
class RecurrentNet():
    def __init__(self, n_inputs, n_hidden, n_outputs,
        ...
        Args:
            device: 计算设备，如 'cuda:0', 'cuda', 'cpu'，None时自动选择
        """
        if device is None:
    ...
    def activate(self, inputs):
        '''
        inputs: (batch_size, n_inputs) - 可以是列表、numpy数组或tensor
        ...
        with torch.no_grad():
            ...
            if isinstance(inputs, list):
                inputs = np.array(inputs)   #转成 NumPy 数组，避免 "Creating a tensor from a list of numpy.ndarrays is extremely slow" 警告
            if isinstance(inputs, np.ndarray):
                inputs = torch.from_numpy(inputs).to(dtype=self.dtype, device=self.device)  #转换成 torch.Tensor,并转到指定的 数据类型和 设备,不会复制数据
            elif isinstance(inputs, torch.Tensor):
                inputs = inputs.to(dtype=self.dtype, device=self.device)    #把 tensor 转到指定的 数据类型和 设备
            else:
                inputs = torch.tensor(inputs, dtype=self.dtype, device=self.device)     #其它格式输入直接创建新的 tensor,会复制数据
            #print(f"neat处理后观测inputs: {inputs}")
